  ...
  rules: [
    ...
    
    {
      ...
      checkFunction: (content, lines, filePath) => {
        ...
        const hasRouteComponent = content.includes('function Page') || 
  ...
  hooks: {
    beforeScan: async (context) => {
      ...
      try {
        ...
        const hasPerformanceTools = performanceDeps.some(dep => 
    ...
    beforeFileProcess: (filePath, content) => {
      ...
      if (content.length > 50000) {
        console.log(`⚠️ Processing large file: ${filePath} (${Math.round(content.length/1024)}KB)`);
      }
    },
  ...
  calculateSeverityScore: function(issues) {
    return issues.reduce((score, issue) => {
      switch (issue.severity) {
        case 'error': return score + 3;
        case 'warning': return score + 2;
