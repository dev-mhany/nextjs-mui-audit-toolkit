...
async function main() {
  try {
    ...
    const existingIndex = report.manualChecks[category].findIndex(
      check => check.route === route
    );
...
function generateManualChecksSection(manualChecks) {
  ...
  const passedChecks = Object.values(manualChecks).flat().filter(c => c.result === 'pass').length;
  const failedChecks = Object.values(manualChecks).flat().filter(c => c.result === 'fail').length;
  const partialChecks = Object.values(manualChecks).flat().filter(c => c.result === 'partial').length;
