...
export class DatabaseService {
  ...
  async createAudit(
    auditData: Omit<AuditResult, 'id' | 'createdAt'>
  ): Promise<AuditResult> {
  ...
  async getAuditsByRepo(repoUrl: string, limit: number = 10): Promise<AuditResult[]> {
    const audits = await this.readAudits()
    return audits.filter(audit => audit.repoUrl === repoUrl).slice(0, limit)
  }
  ...
  async getAuditSummary(): Promise<AuditSummary> {
    ...
    const averageScore =
      completedAuditsWithScores.length > 0
        ? completedAuditsWithScores.reduce((sum, audit) => sum + (audit.score || 0), 0) /
          completedAuditsWithScores.length
        : 0
  ...
  async cleanupOldAudits(daysOld: number = 30): Promise<number> {
    ...
    const filteredAudits = audits.filter(audit => new Date(audit.createdAt) > cutoffDate)
  ...
  > {
    ...
    repoMap.forEach((repoAudits, repoUrl) => {
      const completedAudits = repoAudits.filter(
        audit => audit.status === 'completed' && audit.score !== undefined
      )
      ...
      const sortedAudits = repoAudits.sort(
    ...
    return stats.sort(
      (a, b) => new Date(b.lastAudit).getTime() - new Date(a.lastAudit).getTime()
    )
  }
  ...
  async getInstallation(installationId: number): Promise<GitHubInstallation | null> {
    const installations = await this.readInstallations()
    return (
      installations.find(install => install.installationId === installationId) || null
    )
  }
  ...
  async getAuditsByStatus(status: string, limit: number = 20): Promise<AuditResult[]> {
    const audits = await this.readAudits()
    return audits.filter(audit => audit.status === status).slice(0, limit)
  }
}
