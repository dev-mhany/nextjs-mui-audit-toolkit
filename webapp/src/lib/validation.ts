...
export function validateAuditRequest(request: Record<string, unknown>): AuditRequest {
  ...
  if (request.auditConfig) {
    validated.auditConfig = validateAuditConfig(request.auditConfig as Record<string, unknown>);
  }
  
  return validated;
}

export function validateAuditConfig(config: Record<string, unknown>): Record<string, unknown> {
  ...
  if (config.thresholds && typeof config.thresholds === 'object') {
    ...
    if (typeof (config.thresholds as any).minScore === 'number') {
      if ((config.thresholds as any).minScore < 0 || (config.thresholds as any).minScore > 100) {
...
export function validateWebhookPayload(payload: any): any {
  ...
  const validStatuses = [
    'queued', 'in_progress', 'waiting', 'completed', 'success',
...
export function validateCreateAuditRequest(request: any): any {
  ...
  if (request.mode === 'pat') {
    if (!request.pat) {
      throw new ValidationError('Personal Access Token is required when using PAT mode', 'pat');
    }
