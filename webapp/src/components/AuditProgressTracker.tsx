  ...
  const { 
    ...
    fetcher,
    {
      ...
      onSuccess: (data) => {
        ...
        if (data.audit && (data.audit.status === 'completed' || data.audit.status === 'failed')) {
        ...
        {audit?.score !== undefined && (
          <Box sx={{ mb: 2 }}>
            <Chip
              ...
              color={audit.score >= 85 ? 'success' : audit.score >= 70 ? 'warning' : 'error'}
              variant="outlined"
            />
            {audit.letterGrade && (
              <Chip
